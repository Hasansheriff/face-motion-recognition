...
label=Label(root,text="Camera",bg="white",fg="red").pack()
f1=LabelFrame(root,bg="red")
f1.pack()
L1=Label(f1,bg="red")
L1.pack()
f2=LabelFrame(root,text="emoji",bg="white",fg="red",width=400,height=400)
f2.pack(side=BOTTOM)
L2=Label(f2,bg="red")
L2.pack()
...
def videostream():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            abc=emotion_dict[maxindex]
            frame=cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)
            inputframe=ImageTk.PhotoImage(Image.fromarray(frame))
            L1["image"]=inputframe
            find=emoji_dist[maxindex]
            findimage=Image.open(find)
            findimage=ImageTk.PhotoImage(findimage)
            L2["image"]=findimage
